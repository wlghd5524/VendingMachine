...
public class CollectMoneyPanel extends JPanel {
    ...
    public CollectMoneyPanel() {
        ...
        // 각 화폐를 나타내는 라벨
        String[] moneyLabels = {"10원", "50원", "100원", "500원", "1000원"};
        JLabel[][] labels = new JLabel[moneyLabels.length][2];
        int[][] bounds = {
        ...
        for (int i = 0; i < moneyLabels.length; i++) {
            labels[i][0] = new JLabel(moneyLabels[i]);
            labels[i][0].setHorizontalAlignment(SwingConstants.CENTER);
            labels[i][0].setFont(moneyFont);
            labels[i][0].setBounds(bounds[i][0], bounds[i][1], bounds[i][2], bounds[i][3]);
            add(labels[i][0]);

            labels[i][1] = new JLabel(" X " + MoneyList.moneyList.get(i).getStock());
            labels[i][1].setFont(textFont);
            labels[i][1].setBounds(stockBounds[i][0], stockBounds[i][1], stockBounds[i][2], stockBounds[i][3]);
            add(labels[i][1]);
        }
